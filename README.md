...
git init
git add .
git commit -m "Initial LexiPair app"
git branch -M main
git remote add origin https://github.com/<your-name>/<repo-name>.git
git push -u origin main
```

注意不要提交 `.env`、`server-data`、`node_modules`、`dist`。
...
- 同步后端仍是文件存储
- 没有服务端数据库备份和冲突合并策略
- iOS 上架还需证书、隐私政策、TestFlight 测试

建议下一步：

- 接 Supabase Auth + Postgres
- 给每条学习记录建独立日志表
- 加入同步冲突解决
- 做 iOS 真机测试和 TestFlight 分发
